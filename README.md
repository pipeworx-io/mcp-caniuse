...
`tools/list` at `https://gateway.pipeworx.io/caniuse/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/feature \
  -H 'Content-Type: application/json' \
  -d '{"name":"css-grid"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/feature`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
